...
const PROTEGO_YIELD_VAULT_CORE_ADDRESS =
  "0xf575eE8F71ac2A1A55B2C0B5e5D7d15d73626242";
...
> = ({ onOptimize, onEmergencyWithdrawal }) => {
  ...
  const verifyContracts = async (
    ...
    try {
      ...
      // Test USDC contract
      console.log(
        "Creating USDC contract instance with address:",
        MOCK_USDC_ADDRESS
      );
      ...
      console.log(
        `✅ Contract verified: ${name} (${symbol}) with ${decimals} decimals`
      );
      ...
      setStatusMessage(
        `❌ Contract verification failed: ${error.reason || error.message}`
      );
      throw error;
    }
  };
  ...
  const fetchUSDCBalance = async (
    ...
    try {
      ...
      setStatusMessage(
        `❌ Failed to fetch balance: ${error.reason || error.message}`
      );
  ...
  const connectWallet = useCallback(async () => {
    ...
    try {
      ...
      if (network.chainId !== SEI_TESTNET_CONFIG.chainId) {
        console.log(
          `Wrong network detected. Expected: ${SEI_TESTNET_CONFIG.chainId}, Got: ${network.chainId}`
        );
        setStatusMessage(
          `Wrong network detected (Chain ID: ${network.chainId}). Switching to DuckChain Testnet...`
        );
        const switched = await switchToSeiTestnet();
        if (!switched) {
          setStatusMessage(
            `Please manually switch to DuckChain Testnet (Chain ID: ${SEI_TESTNET_CONFIG.chainId})`
          );
      ...
      try {
        ...
        const { decimals, symbol, contract } =
          await verifyContracts(web3Provider);

        console.log("Starting balance fetch...");
        await fetchUSDCBalance(addr, contract, decimals);

        setStatusMessage(
          `✅ Connected to DuckChain Testnet | ${addr.substring(0, 6)}...${addr.substring(addr.length - 4)}`
        );
      } catch (contractError) {
        console.error("Contract operations failed:", contractError);
        setStatusMessage(
          `⚠️ Wallet connected but contract verification failed. Check contract addresses.`
        );
      }
      ...
      if (walletAddress && contractInfo && provider) {
        try {
          ...
          await fetchUSDCBalance(
            walletAddress,
            usdcContract,
            parseInt(contractInfo.decimals)
          );
        } catch (error) {
    ...
    try {
      ...
      try {
        ...
        console.log(
          ...
          ethers.utils.formatUnits(currentAllowance, contractInfo.decimals)
        );
  ...
  const handleDeposit = async () => {
    ...
    try {
      const yieldVaultCoreContract = new ethers.Contract(
        PROTEGO_YIELD_VAULT_CORE_ADDRESS,
      ...
      console.log(
        `Depositing ${depositAmount} USDC (${amountToDeposit.toString()} wei)`
      );

      const tx = await yieldVaultCoreContract.deposit(
        amountToDeposit,
        walletAddress,
        {
